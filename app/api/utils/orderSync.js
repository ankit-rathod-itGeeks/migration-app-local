...
function writeOrdersReportXlsx({ reportRows, reportDir, filePrefix = "orders_migration_report" }) {
  ensureDir(reportDir);
  ...
  const filename = `${safeFilePart(filePrefix)}_${stamp}.xlsx`;
...
export async function migrateOrdersFromSheet(fileBuffer) {
  try {
    ...
    for (const parsedOrder of parsedOrders) {
      ...
      if (result.success) {
        ...
        reportRows.push({
          ...
          sourceOrderId: parsedOrder.sourceId,
          customerEmail: parsedOrder.customerEmail || "",
          status: "SUCCESS",
          targetOrderName: result.orderName || "",
    ...
    const reportPath = writeOrdersReportXlsx({
      reportRows,
      reportDir,
      filePrefix: `orders_migration_report_${safeFilePart(TARGET_SHOP)}`,
    });

    // SAME RESULT FORMAT AS PRODUCTS/CUSTOMERS (ADDED)
    const createdCount = successCount;
    const skippedCount = 0;
    const failedCount = failureCount;
