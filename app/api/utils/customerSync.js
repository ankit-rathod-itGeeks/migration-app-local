...
/**
 * REPORT HELPERS (same pattern as products)
 */
...
function loadRows(fileBuffer) {
  ...
  if (!sheetName) {
    throw new Error(
      `Missing sheet "Customers" (or "Customer"). Found: ${wb.SheetNames.join(", ")}`
    );
  }
...
function makeCustomerKey(row) {
  ...
  if (!isEmpty(fn) || !isEmpty(ln)) {
    return `name:${String(fn || "").trim().toLowerCase()}::${String(ln || "")
      .trim()
      .toLowerCase()}`;
  }
  return null;
...
function buildCustomersFromRows(rows, detectedMetafields) {
  ...
  for (const row of rows) {
    ...
    if (isEmpty(c.multipassIdentifier) && !isEmpty(row["Multipass Identifier"]))
      c.multipassIdentifier = row["Multipass Identifier"];

    if (isEmpty(c.emailMarketingStatus) && !isEmpty(row["Email Marketing: Status"]))
      c.emailMarketingStatus = row["Email Marketing: Status"];
    if (isEmpty(c.emailMarketingLevel) && !isEmpty(row["Email Marketing: Level"]))
      c.emailMarketingLevel = row["Email Marketing: Level"];
    if (isEmpty(c.emailMarketingUpdatedAt) && !isEmpty(row["Email Marketing: Updated At"]))
      c.emailMarketingUpdatedAt = row["Email Marketing: Updated At"];

    if (isEmpty(c.smsMarketingStatus) && !isEmpty(row["SMS Marketing: Status"]))
      c.smsMarketingStatus = row["SMS Marketing: Status"];
    if (isEmpty(c.smsMarketingLevel) && !isEmpty(row["SMS Marketing: Level"]))
      c.smsMarketingLevel = row["SMS Marketing: Level"];
    if (isEmpty(c.smsMarketingUpdatedAt) && !isEmpty(row["SMS Marketing: Updated At"]))
      c.smsMarketingUpdatedAt = row["SMS Marketing: Updated At"];
    ...
    if (hasAnyAddressData(row)) {
      const addrFirstName = !isEmpty(row["Address First Name"])
        ...
        : c.firstName;
      const addrLastName = !isEmpty(row["Address Last Name"])
        ? row["Address Last Name"]
        : c.lastName;
      const addrPhone = normalizePhone(row["Address Phone"]) || c.phone;
